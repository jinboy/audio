...
namespace NAudioDemo.RecordingDemo
{
    public partial class RecordingPanel : UserControl
    {
        ...
        public RecordingPanel()
        {
            ...
            // step1: 首先，让我们选择将录制的音频放在何处。它将转到桌面上recorded.wav一个NAudioDemo文件夹中的文件
        ...
        private string GetFileName()
        {
            ...
            var sampleRate = string.Format("{captureDevice.WaveFormat.SampleRate / 1000}kHz");
            var channels = captureDevice.WaveFormat.Channels == 1 ? "mono" : "stereo";

            return string.Format("{deviceName} {sampleRate} {channels} {DateTime.Now:yyy-MM-dd HH-mm-ss}.wav");
        }
